...
fn main() {
    ...
    for _ in 0..1 {
        if let Ok(packet) = cap.next_packet() {
            ...
            println!("options {}", ipv4_packet.options[0].size);

            println!("src mac {}", ipv4_packet.eth_frame.to_string_src_mac());
            println!("dst mac {}", ipv4_packet.eth_frame.to_string_dest_mac());
            println!("src addr {}", ipv4_packet.eth_frame.to_string_src_mac());
            println!("dst addr {}", ipv4_packet.eth_frame.to_string_dest_mac());
